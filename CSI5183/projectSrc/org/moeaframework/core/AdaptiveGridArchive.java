...
public class AdaptiveGridArchive extends NondominatedPopulation {
	...
	public Solution[] getParents() {	
		...
		for (int i = 0; i < parents.length; i++) {
			Solution temp1 = get((int) Math.floor(Math.random() * size()));
			Solution temp2 = get((int) Math.floor(Math.random() * size()));

			while (temp1 == temp2) {
				temp2 = get((int) Math.floor(Math.random() * size()));
			}
			
			int val1 = density[findIndex(temp1)];
			int val2 = density[findIndex(temp2)];
