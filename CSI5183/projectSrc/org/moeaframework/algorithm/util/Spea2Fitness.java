...
public class Spea2Fitness {
	 ...
	 */
	private Population solutionSet_ = null;
	...
	private static final ParetoObjectiveComparator dominance_ = new ParetoObjectiveComparator();
...
//			solutionSet_.get(i).setFitness(rawFitness[i] + kDistance);
			solutionSet_.get(i).setAttribute("fitness", rawFitness[i] + kDistance);
		} // for                  
	} // fitnessAsign
	 ...
	 */
	public Population environmentalSelection(int size){        
		...
		while (i < solutionSet_.size()){
			if ((double) solutionSet_.get(i).getAttribute("fitness")<1.0){
...
//			aux.get(pos).setLocation(pos);
			aux.get(pos).setAttribute("location", i);
...
//			int tmp = aux.get(toRemove).getLocation();
			int tmp = (int) aux.get(toRemove).getAttribute("location");
			aux.remove(toRemove);            
