...
public class Distance {
	...
	public void crowdingDistanceAssignment(SolutionSet solutionSet, int nObjs, int typeSwitch, SolutionSet archive) {

		if (archive==null && (typeSwitch==3 || typeSwitch==4 || typeSwitch==6) ) {
		...
		case 1:
			...
			crowdingDistanceAssignmentToExtremeValues(solutionSet, nObjs, false, null);
			break;
		case 2:
			// sum of distances to max values per objective (scaled)
			crowdingDistanceAssignmentToExtremeValues(solutionSet, nObjs, true, null);
			break;

		case 3:
			// sum of distances to max values per objective (w.r.t. archive)
			crowdingDistanceAssignmentToExtremeValues(solutionSet, nObjs, false, archive);
			break;
		case 4:
			// sum of distances to max values per objective (w.r.t. archive, scaled)
			crowdingDistanceAssignmentToExtremeValues(solutionSet, nObjs, true, archive);
			break;
	...
	public void crowdingDistanceAssignmentAGE(SolutionSet solutionSet, int nObjs) {
		...
		for (int i = 0; i<nObjs; i++) {          
			...
			objetiveMinn = front.get(0).getObjective(i);      
			objetiveMaxn = front.get(front.size()-1).getObjective(i);    
			//      System.out.println(objetiveMinn + " " + objetiveMaxn);                  // min is 0th element, max is nth element
			...
			for (int j = 1; j < size-1; j++) {
				...
				int pos = j;
				//          if (pos>=middle) {   /*do not do this! we go through each objectiv: why include a bad one again?*/                           // why this in combination with falling probability?
				////              System.out.print("!");
				//              pos = size-j-1;
				//          }
				if (debugPrint) System.out.print(" pos="+pos+ " ");

				double rand = Math.random();

				if (debugPrint) System.out.print(front.get(j).getObjective(i));
				//          if (Math.random()< (1d/ (pos) )  ) front.get(j).setCrowdingDistance(Double.POSITIVE_INFINITY);//falling probability toward the center of the front
				/*use*/   if (rand< (1d/ (pos+1) )  )  {
					//           if (rand< (1d)  )  {
					front.get(j).setCrowdingDistance(Double.POSITIVE_INFINITY);  //falling probability toward the center of the front
					if (debugPrint) System.out.println(" {"+rand+"<"+1d/ (pos+1)+"} y");
				} else {
					if (front.get(j).getCrowdingDistance()!=Double.POSITIVE_INFINITY) front.get(j).setCrowdingDistance(0); // reset to zero if not already INFINITY
					if (debugPrint) System.out.println();
				}
				//          if (Math.random()< 1d/nObjs  ) front.get(j).setCrowdingDistance(Double.POSITIVE_INFINITY);//1/d probability     DTLZ3_3D=eps3
				//          if (Math.random()< .5  ) front.get(j).setCrowdingDistance(Double.POSITIVE_INFINITY);//50% probability     DTLZ3_3D=eps2
				//          if (Math.random()< 1  ) front.get(j).setCrowdingDistance(size-pos + front.get(j).getCrowdingDistance());

				//          if (Math.random()< 1  ) front.get(j).setCrowdingDistance(Double.POSITIVE_INFINITY);//100% probability     DTLZ3_3D=eps4
	...
	/* x_i = max { (max{y\inY} f_i(y)) - f_i(x) ,0}
	 * (code based on crowdingDistanceAssignment/2)
	 */
	public void crowdingDistanceAssignmentToExtremeValues(SolutionSet solutionSet, int nObjs, boolean scale, SolutionSet archive) {
		int size = solutionSet.size();        
		...
		for (int i = 0; i<nObjs; i++) {          
			...
			for (int j = 1; j < size-1; j++) {
				...
				//	        distance = Math.max( front.get(front.size()-1).getObjective(i) / front.get(j).getObjective(i) , 1); // trial
				distance = Math.max( front.get(front.size()-1).getObjective(i) - front.get(j).getObjective(i) , 0);
	...
	public void crowdingDistanceAssignmentRanking(SolutionSet solutionSet, int nObjs, SolutionSet archive) {
		int size = solutionSet.size();        
